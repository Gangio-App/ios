...
let logger = Logger(subsystem: "chat.gangio.app", category: "settingsStore")
...
class NotificationOptionsData: Codable {
    ...
    init(keyWasSet: @escaping () -> Void) {
        self.rejectedRemoteNotifications = false
...
class UserSettingsData {
    ...
    static var cacheFile: URL? {
        if let caches = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            let gangioDir = caches.appendingPathComponent(Bundle.main.bundleIdentifier!, conformingTo: .directory)
            let resp = gangioDir.appendingPathComponent("userInfoCache", conformingTo: .json)
            return resp
        }
        return nil
    }
    ...
    static var storeFile: URL? {
        if let caches = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            let gangioDir = caches.appendingPathComponent(Bundle.main.bundleIdentifier!, conformingTo: .directory)
            let resp = gangioDir.appendingPathComponent("userSettings", conformingTo: .json)
            return resp
        }
        return nil
    }
    ...
    func writeCacheToFile() {
        DispatchQueue.main.async(qos: .utility) {
            if let caches = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
                let gangioDir = caches.appendingPathComponent(Bundle.main.bundleIdentifier!, conformingTo: .directory)
                do {
                    try FileManager.default.createDirectory(at: gangioDir, withIntermediateDirectories: true)
                } catch {}
