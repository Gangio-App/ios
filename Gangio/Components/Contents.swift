...
//        let nsattr = NSAttributedString(string: "Zomatree", attributes: [.foregroundColor: UIColor.red, .font: font, .link: "https://gangio.chat"])
...
//
//                        mention = NSAttributedString(string: "#\(name)", attributes: [.foregroundColor: viewState.theme.accent.color, .font: boldFont, .link: "gangiochat://channels?channel=\(string)"])
//                    } else {
...
struct InnerContents: UIViewRepresentable {
    ...
    func updateUIView(_ textview: UIViewType, context: Context) {
        if context.coordinator.lastContent == content && context.coordinator.lastFontSize == fontSize {
            InnerContents.recalculateHeight(view: textview, result: $calculatedHeight)
            return
        }
        ...
        for match in attrString.string.matches(of: /(?:https?:\/\/)?(?:gangio|gangio)\.chat\/server\/(\w{26})\/channel\/(\w{26})\/(\w{26})/).reversed() {
               ...
               channel.server == serverId
            {
                ...
                linkString.addAttributes([
                    .link: URL(string: "gangiochat://channels?channel=\(channelId)&message=\(messageId)")!,  // TODO
    ...
    static func recalculateHeight(view: UIView, result: Binding<CGFloat>) {
        let width = view.frame.width > 0 ? view.frame.width : UIScreen.main.bounds.width - 32
        let newSize = view.sizeThatFits(CGSize(width: width, height: .greatestFiniteMagnitude))
        
        guard newSize.height > 0 && newSize.height < .greatestFiniteMagnitude else { return }
        guard abs(result.wrappedValue - newSize.height) > 1.0 else { return }
