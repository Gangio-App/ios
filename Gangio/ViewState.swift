...
public class AppViewState: ObservableObject {
    ...
    let keychain = Keychain(service: "chat.gangio.app")
    var http: HTTPClient
    var launchTransaction: any Sentry.Span
    
    // MARK: - Debounced Persistence
    /// Tracks which keys need to be persisted
    private var dirtyKeys: Set<String> = []
    /// Timer for debounced writes
    private var persistenceTimer: Timer?
    /// Debounce interval in seconds
    private let persistenceDebounceInterval: TimeInterval = 2.0
    
    /// Mark a key as needing persistence and schedule a debounced write
    ...
    private func schedulePersistence() {
        ...
        persistenceTimer = Timer.scheduledTimer(withTimeInterval: persistenceDebounceInterval, repeats: false) { [weak self] _ in
            self?.flushPersistence()
        }
    }
    
    /// Persist all dirty keys to UserDefaults
    func flushPersistence() {
        ...
        // Capture values on main thread
        let encoder = JSONEncoder()
        var dataToWrite: [(String, Data)] = []
        
        for key in keysToFlush {
            do {
                switch key {
                case "users":
                    dataToWrite.append((key, try encoder.encode(users)))
                case "servers":
                    dataToWrite.append((key, try encoder.encode(servers)))
                case "channels":
                    dataToWrite.append((key, try encoder.encode(channels)))
                case "messages":
                    // Only persist last 200 messages per channel to avoid huge writes
                    let recentMessages = limitMessagesForPersistence()
                    dataToWrite.append((key, try encoder.encode(recentMessages)))
                case "channelMessages":
                    let limitedChannelMessages = channelMessages.mapValues { Array($0.suffix(200)) }
                    dataToWrite.append((key, try encoder.encode(limitedChannelMessages)))
                case "members":
                    dataToWrite.append((key, try encoder.encode(members)))
                case "dms":
                    dataToWrite.append((key, try encoder.encode(dms)))
                case "emojis":
                    dataToWrite.append((key, try encoder.encode(emojis)))
                case "currentUser":
                    dataToWrite.append((key, try encoder.encode(currentUser)))
                case "apiUrl":
                    dataToWrite.append((key, try encoder.encode(apiUrl)))
                case "apiInfo":
                    dataToWrite.append((key, try encoder.encode(apiInfo)))
                default:
                    break
                }
            } catch {
                print("Failed to encode \(key) for persistence: \(error)")
            }
        }
        
        // Write on background thread
        DispatchQueue.global(qos: .utility).async {
            for (key, data) in dataToWrite {
                UserDefaults.standard.set(data, forKey: key)
            }
        }
    }
    
    /// Limit persisted messages to only those referenced by channelMessages (last 200 per channel)
    private func limitMessagesForPersistence() -> [String: Message] {
        var recentIds = Set<String>()
        for (_, ids) in channelMessages {
            for id in ids.suffix(200) {
                recentIds.insert(id)
            }
        }
        return messages.filter { recentIds.contains($0.key) }
    }
...
//        self.currentChannel = .home
        ...
        self.users["00000000000000000000000000"] = User(id: "00000000000000000000000000", username: "Gangio", discriminator: "0000")
    ...
    static func preview() -> AppViewState {
        ...
        this.apiInfo = ApiInfo(
            ...
            features: ApiFeatures(
                ...
                email: true,
                invite_only: false,
                autumn: GangioFeature(enabled: true, url: "https://cdn.gangiousercontent.com"),
                january: GangioFeature(enabled: true, url: "https://proxy.gangiousercontent.com"),
    ...
    func signIn(mfa_ticket: String, mfa_response: [String: String], callback: @escaping((LoginState) -> ())) async {
        let body = ["mfa_ticket": mfa_ticket, "mfa_response": mfa_response, "friendly_name": "Gangio iOS"] as [String : Any]

        await innerSignIn(body, callback)
    }

    func signIn(email: String, password: String, callback: @escaping((LoginState) -> ())) async {
        let body = ["email": email, "password": password, "friendly_name": "Gangio IOS"]
    ...
    func destroyCache() {
        // Cancel any pending persistence writes
        persistenceTimer?.invalidate()
    ...
    func backgroundWsTask() async {
        ...
        // Retry API info fetch up to 3 times with backoff
        var apiInfoFetched = false
        for attempt in 0..<3 {
            do {
                ...
                print("API info fetch attempt \(attempt + 1) failed: \(error)")
                if attempt < 2 {
        ...
        if !apiInfoFetched {
            if self.apiInfo != nil {
                // Use cached apiInfo after fetch failure
                print("Using cached API info after fetch failure")
            } else {
                state = .connecting
        ...
        let ws = WebSocketStream(url: self.apiInfo!.ws, token: token, onEvent: onEvent)
        self.ws = ws
    }
    ...
    func onEvent(_ event: WsMessage) async {
        ...
        switch event {
            case .ready(let event):
                ...
                if !isPreview {
                    ...
                    if let savedToken = AppDelegate.lastPushToken {
                        Task {
                            debugPrint("Re-uploading saved push token on reconnect")
                            _ = await http.uploadNotificationToken(token: savedToken)
                        }
                    }
                }
                ...
                // Only clear channel messages if the cached data is very stale
                // (more than 100 messages behind) — otherwise keep the cache
                // This prevents the "data keeps getting deleted" issue
                for channel in channels.values {
                    ...
                    {
                        // Keep existing cache — new messages will arrive via WebSocket
                        // Only clear if we have no cached messages at all
                        if channelMessages[channel.id]?.isEmpty ?? true {
                            channelMessages[channel.id] = []
                        }
                    }
                }
            ...
            case .message(let message):
                ...
                if channelMessages[message.channel] == nil {
                    channelMessages[message.channel] = []
                }
                
                // Ensure the message is not already in the list to prevent duplicate ID crashes
                if !channelMessages[message.channel]!.contains(message.id) {
                    channelMessages[message.channel]!.append(message.id)
                }
