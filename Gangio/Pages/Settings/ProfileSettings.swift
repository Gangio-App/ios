                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        VStack(spacing: 0) {
                            ...
                            Button {
                                Task {
                                    do {
                                        let _ : Types.User = try await viewState.http.req(method: .patch, route: "/users/@me", parameters: ["profile": ["content": profile?.content]]).get()
        ...
        .onChange(of: bannerItem) { _, newItem in
            ...
            Task { @MainActor in
                ...
                do {
                    ...
                    let _ : Types.User = try await viewState.http.req(method: .patch, route: "/users/@me", parameters: ["profile": ["background": uploadResp.id]]).get()
                    profile = try? await viewState.http.fetchProfile(user: viewState.currentUser!.id).get()
